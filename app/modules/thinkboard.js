    ...
    datas.forEach(function (data) {
        ...
        switch (readData[0]) {
            case self.sensorValueSize.FLOAT: {
                //
                if (type === self.sensorTypes.TEMP) {
                    // Add for TEMP Sensor
                    for (let i = 0; i < 4; i++) {
                        rhumi[i] = new Buffer(
                            readData.subarray(1 + i * 10, 5 + i * 10)
                        ).readFloatLE();
                        rhumi[i] = Math.round(rhumi[i] * 100) / 100;
                        rtemp[i] = new Buffer(
                            readData.subarray(6 + i * 10, 10 + i * 10)
                        ).readFloatLE();
                        rtemp[i] = Math.round(rtemp[i] * 100) / 100;
                    }
                } else if (type === self.sensorTypes.USONIC) {
                    // Add for USONIC Sensor
                    for (let i = 0; i < 4; i++) {
                        rtemp[i] = new Buffer(
                            readData.subarray(1 + i * 5, 5 + i * 5)
                        ).readFloatLE();
                        rtemp[i] = Math.round(rtemp[i] * 100) / 100;
                    }
                } else {
